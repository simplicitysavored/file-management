...
public class BaseBean {
    ...
    public static BaseBean initialByFile(Class<? extends BaseBean> clz, File file) throws IllegalAccessException, InstantiationException, UnsupportedEncodingException {
        ...
        if (null != prev) {
            ...
            baseBean.setPreviousUrl(NasUtil.systemProperty().getServerProtocol() + NasUtil.systemProperty().getServerHost() +":" + NasUtil.systemProperty().getServerPort() + "/nas?path=" + URLEncoder.encode(prev.getAbsolutePath(), "UTF-8"));
            baseBean.setProtect(NasUtil.isProtectFile(file));
        }
