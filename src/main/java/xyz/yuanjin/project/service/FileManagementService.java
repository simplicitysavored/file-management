...
public class FileManagementService {
     ...
     * @throws Exception 异常
     */
    public FolderBean loadFolder(YjFile file, Boolean dotBack) throws Exception {
        if (!file.exists()) {
        ...
        YjFile[] tmpFiles = file.listFiles();

        if (null != tmpFiles) {

            for (YjFile tmpFile : tmpFiles) {
    ...
    public boolean isProtectFile(YjFile file) {
        return SystemUtil.isProtectFile(file);
    }
    ...
    public YjFile checkFilePathV2(String path) throws Exception {
...
//        SystemConfig.getInstance().getListenFolderMap().forEach((alias, file)-> dtoList.add(new DropdownDTO(file.getAbsolutePath(), alias)));
        SystemConfig.getInstance().getListenFolderIdAliasMap().forEach((id, alias) -> dtoList.add(new DropdownDTO(id, alias)));
        return dtoList;
    }
}
